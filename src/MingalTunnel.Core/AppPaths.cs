...
{
    /// <summary>Overridable (MINGALTUNNEL_DATA) so tests never touch the real profile.</summary>
    public static string DataDir { get; set; } =
        Environment.GetEnvironmentVariable("MINGALTUNNEL_DATA") is { Length: > 0 } d
            ? d
            : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "MingalTunnel");
