...
public static class AppCatalog
{
    ...
    public static string DisplayPath(TunneledApp a) => FriendlyPath(a.Kind switch
    {
    ...
    /// <summary>
    /// Shows per-user folders as %LOCALAPPDATA%, %APPDATA% or %USERPROFILE%, so
    /// the Windows user name never ends up on screen (or in a screenshot).
    /// </summary>
    public static string FriendlyPath(string path)
    {
        foreach (var (folder, token) in new[]
                 {
                     (LocalAppData, "%LOCALAPPDATA%"),
                     (RoamingAppData, "%APPDATA%"),
                     (Env(Environment.SpecialFolder.UserProfile), "%USERPROFILE%"),
                 })
        {
